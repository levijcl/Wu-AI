...
class BlackjackEnv(gym.Env):
    ...
    ### Observation Space
    The observation is a 3D vector, representing:
    - The player's current sum (0-31)
    - The dealer's shown card (1-10)
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, natural=False, sab=False):
        ...
        # Use Dict space instead of Tuple for SB3 compatibility
        self.observation_space = spaces.Dict({
            "player_sum": spaces.Box(low=0, high=31, shape=(1,), dtype=np.float32),
            "dealer_card": spaces.Box(low=1, high=10, shape=(1,), dtype=np.float32),
            "usable_ace": spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32)
        })
    ...
    def _get_obs(self):
        ...
        has_usable_ace = usable_ace(self.player)

        # Return a dictionary observation
        return {
            "player_sum": np.array([player_sum], dtype=np.float32),
            "dealer_card": np.array([dealer_card], dtype=np.float32),
            "usable_ace": np.array([float(has_usable_ace)], dtype=np.float32)
        }
    ...
    def render(self):
        ...
        obs = self._get_obs()
        player_sum = int(obs["player_sum"][0])
        dealer_card_value = int(obs["dealer_card"][0])
        usable_ace = bool(obs["usable_ace"][0])
...
# Pixel art from Mariia Khmelnytska (https://www.123rf.com/photo_104453049_stock-vector-pixel-art-playing-cards-standart-deck-vector-set.html/Users/levi/Downloads/gym-master/gym/envs/toy_text/font/Minecraft.ttf)